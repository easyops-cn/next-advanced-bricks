...
import type { RangeTuple, TransformLiteral/* , ActiveTarget */ } from "./interfaces";
import {
      ...
      rootSelection.on(
        ...
        (e: WheelEvent & { wheelDeltaX: number; wheelDeltaY: number }) => {
          ...
          if (!e.ctrlKey) {
            ...
            if (scrollable) {
              const node = (e.target as HTMLElement)!.closest(".node-online-search");
              if (node) {
                if (checkScrollable(node, e.deltaY)) {
                  return;
                }
              }
...
// 检查元素是否还能滚动
function checkScrollable(element: Element, delta: number) {
