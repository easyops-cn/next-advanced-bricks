...
```tsx
import { WrappedEoMiniLineChart } from "@easyops/wrapped-components";
```
...
```tsx
{
  ["var(--palette-orange-5)", "darkcyan", "darkred"].map((color) => (
    <WrappedEoMiniLineChart
      key={color}
      xField="time"
      ...
      data={[
        ...
        { request_total: 556.57, time: 1730613600 },
      ]}
    />
  ));
}
