...
```tsx
import { WrappedNavLaunchpadConfig } from "@easyops/wrapped-components";
```
...
| 事件            | detail                                                                                   | 说明                   |
| --------------- | ---------------------------------------------------------------------------------------- | ---------------------- |
| onMenuItemClick | `ConfigMenuItemNormal` — 被点击的菜单项数据（app 或 custom 类型）                        | 点击菜单项时触发       |
| onActionClick   | `MenuActionEventDetail` — { data: 操作关联的菜单项或分组, action: 被点击的操作按钮配置 } | 点击菜单操作按钮时触发 |
