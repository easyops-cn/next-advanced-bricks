...
# WrappedVisualBuilderPreGeneratedConfig
...
```tsx
import { WrappedVisualBuilderPreGeneratedConfig } from "@easyops/wrapped-components";
```
...
| 事件          | detail                                                                  | 说明                                                 |
| ------------- | ----------------------------------------------------------------------- | ---------------------------------------------------- |
| onBrickChange | `BrickConf \| null` — 根据当前选择和权重生成的砖块配置，无选中时为 null | 用户选择、排序或调整权重后，生成的砖块配置变化时触发 |
...
<WrappedVisualBuilderPreGeneratedConfig
  attrList={[
    { id: "name", name: "名称", candidates: [{ visualWeight: 0 }] },
    { id: "status", name: "状态", candidates: [{ visualWeight: 0 }] },
    { id: "createTime", name: "创建时间", candidates: [{ visualWeight: 0 }] },
  ]}
  mockList={[
    { name: "主机-001", status: "运行中", createTime: "2024-01-01" },
    { name: "主机-002", status: "已停止", createTime: "2024-01-02" },
  ]}
  containerConfig={{
    type: "table",
    dataName: "hostList",
  }}
  onBrickChange={(e) => console.log(e.detail)}
/>
```
...
<WrappedVisualBuilderPreGeneratedConfig
  attrList={[
    { id: "name", name: "名称", candidates: [{ visualWeight: 0 }] },
    { id: "ip", name: "IP 地址", candidates: [{ visualWeight: 0 }] },
    { id: "status", name: "运营状态", candidates: [{ visualWeight: 0 }] },
  ]}
  mockList={[{ name: "主机-001", ip: "192.168.1.1", status: "运行中" }]}
  containerConfig={{
    type: "descriptions",
    dataName: "hostDetail",
  }}
