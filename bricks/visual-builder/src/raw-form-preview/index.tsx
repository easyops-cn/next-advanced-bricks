...
import { convertToForm } from "./convert";
import styleText from "../raw-data-preview/styles.shadow.css";
...
export interface RawFormPreviewProps {
  previewUrl?: string;
...
/**
 * 构件 `visual-builder.raw-form-preview`
 *
 * @internal
 */
export
@defineElement("visual-builder.raw-form-preview", {
  styleTexts: [styleText],
})
class RawFormPreview extends ReactNextElement {
  @property()
  ...
  useEffect(() => {
    if (ready) {
      ...
      const onMessage = ({ data }: MessageEvent<PreviewMessage>) => {
        if (data?.channel === "raw-form-preview") {
          switch (data.type) {
            case "comment":
  ...
  useEffect(() => {
    ...
    const table: BrickConf & { context?: ContextConf[] } = {
      ...
      properties: {
        style: {
          gridTemplateColumns:
            "minmax(120px, 0.5fr) 32px auto repeat(4, 1fr) auto 1fr",
        },
      },
    ...
    for (let i = 0, size = generations.length; i < size; i++) {
      ...
      const candidatesByVisualWeight = new Map<number, FormConfig>();
    ...
    render(
        ...
        yaml: safeDump(
          [
            table,
            {
              ...
              events: {
                message: {
                  if: "<% EVENT.detail.data?.channel === 'raw-form-preview' && EVENT.detail.data.type === 'busy' %>",
                  action: "context.replace",
      ...
      },
      {
        ...
        styleText: sharedPreviewStyleText,
      }
    );
